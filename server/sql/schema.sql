...
CREATE OR REPLACE FUNCTION mirr_add_bypath_noins(arg_serverid integer, arg_path text) RETURNS integer AS $$
DECLARE
    fileid integer;
    arr smallint[];
BEGIN
...
EXCEPTION
    WHEN unique_violation THEN
        RAISE NOTICE 'file % was just inserted by somebody else', arg_path;
        -- just update it by calling ourselves again
        SELECT into fileid mirr_add_bypath_noins(arg_serverid, arg_path);
        RETURN fileid;
END;
$$ LANGUAGE plpgsql;
